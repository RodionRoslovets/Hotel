window.addEventListener('DOMContentLoaded', ()=>{
    let starLine = document.getElementsByClassName('rateBtns__line'),
        star = document.getElementsByClassName('rateBtns__star');
        ...
        for(let k = 0; k < starLine[i].children.length; k++){
            starLine[i].children[k].addEventListener('click', ()=>{
                for(var z = 0; z <= k; z++){
                    ...
                    starLine[i].children[z].classList.add('rateBtns__star-icon__active');
                }
                for(let j = z ; j < 5; j++){
                    ...
                    starLine[i].children[j].classList.remove('rateBtns__star-icon__active');
                }
            });
        }
    }
